...
-- lua/configs/cmake-tools.lua

local function setup_cmake()
    local overseer_opts = {
        new_task_opts = {},
        on_new_task = function(_) require('overseer').open { enter = false, direction = 'bottom' } end,
    }
    ...
    } -- Centralized setup as discussed
end

-- Use a specific autocmd group so you can clear it if needed
local augroup = vim.api.nvim_create_augroup('CMakeLazyLoad', { clear = true })
...
vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function()
        ...
        if not package.loaded['cmake-tools'] then setup_cmake() end
        -- Remove the autocmd after the first trigger so it doesn't run again
        vim.api.nvim_clear_autocmds { group = 'CMakeLazyLoad' }
    end,
})
