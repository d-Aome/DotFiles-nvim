...
-- Native implementation of lazy.nvim's `event = 'LspAttach'`
vim.api.nvim_create_autocmd('LspAttach', {
    -- We use a group to keep things organized, though not strictly required
    group = vim.api.nvim_create_augroup('TinyDiagnosticLazyLoad', { clear = true }),
    callback = function()
        -- Ensure we only run the setup and diagnostic override once
        if not package.loaded['tiny-inline-diagnostic'] then
            require('tiny-inline-diagnostic').setup(M.opts)

            -- Disable standard virtual text so they don't overlap
